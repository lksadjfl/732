...
class OdomLogger(Node):
    ...
    def compute_stats(self):
        ...
        if self.mode == 'linear':
            ...
            # Correct square-path closing error:
            # distance from final pose back to THIS TRIAL'S start pose
            delta_x = last['x'] - first['x']
            delta_y = last['y'] - first['y']
            closing_error = math.sqrt(delta_x ** 2 + delta_y ** 2)
            ...
            return {
                ...
                'final_x': round(delta_x, 4),
                'final_y': round(delta_y, 4),
    ...
    def plot(self, path):
        ...
        fig.suptitle(
            f'{title} | TurtleBot 4 | Namespace: {self.namespace}',
            fontsize=14,
            fontweight='bold'
        )
        ...
        ax_traj.plot(x0, y0, 'go', markersize=12, label='Start', zorder=5)
        ax_traj.plot(
            x_end, y_end, 'r^', markersize=12,
            label=f'End ({x_end:.3f}, {y_end:.3f})',
            zorder=5
        )

        if self.mode == 'linear':
            # Draw ideal straight line from THIS trial's start pose
            ax_traj.plot(
                [x0, x0 + self.target],
                [y0, y0],
                'k--',
                linewidth=1.2,
                label=f'Ideal {self.target}m straight'
            )

            actual_displacement = math.sqrt(
                (x_end - x0) ** 2 + (y_end - y0) ** 2
            )
            ax_traj.set_title(
                f'XY Trajectory\nTarget: {self.target}m  |  Actual: {actual_displacement:.4f}m',
                fontsize=12
            )

        else:
            # Draw ideal 1m square from THIS trial's start pose
            sq = plt.Polygon(
            ...
            closing = math.sqrt((x_end - x0) ** 2 + (y_end - y0) ** 2)
            ax_traj.plot(
                [x_end, x0], [y_end, y0],
                'r--',
                linewidth=1,
        ...
        fig.text(
            0.01, 0.01,
            f"Samples: {len(self.data)}  |  Duration: {times[-1]:.1f}s  |  Namespace: {self.namespace}",
            fontsize=8,
            color='gray'
        )
